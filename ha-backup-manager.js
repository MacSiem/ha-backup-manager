/* HA Tools split — ha-backup-manager v4.1.5 (2026-08-28) — single-tool standalone repo */
(function() {
'use strict';
...
const haToolsPersistence = { _cache: {}, _hass: null, setHass(h) { this._hass = h; }, async save(k, d) { try { localStorage.setItem('ha-backup-manager-' + k, JSON.stringify(d)); } catch(e) { console.debug('[ha-backup-manager] caught:', e); } }, async load(k) { try { const r = localStorage.getItem('ha-backup-manager-' + k); return r ? JSON.parse(r) : null; } catch(e) { return null; } }, loadSync(k) { try { const r = localStorage.getItem('ha-backup-manager-' + k); return r ? JSON.parse(r) : null; } catch(e) { return null; } } };

/* ===== HA Tools split — inline shared infrastructure ===== */
// Bento Design System CSS (inline copy — keeps tool standalone)
const HA_BACKUP_MANAGER_BENTO_CSS = `
/* ═══════════════════════════════════════════════
...
class HaBackupManager extends HTMLElement {
  ...
  _renderFrequencyChart() {
    const L = this._lang === 'pl';
    const values = this._healthData.weeklyData.map(value => {
    ...
    const summaries = values.map((value, index) => {
      const week = index + 1;
      const weekLabel = L ? `Tydz. ${week}` : `Week ${week}`;
      const valueLabel = L
        ? `${value} ${value === 1 ? 'kopia' : 'kopii'}`
        : `${value} ${value === 1 ? 'backup' : 'backups'}`;
      return `${weekLabel}: ${valueLabel}`;
    });
    const chartLabel = L
      ? `Liczba kopii zapasowych w ostatnich czterech tygodniach. ${summaries.join('; ')}`
      : `Backup count over the last four weeks. ${summaries.join('; ')}`;

    return `
      <div id="frequency-chart" class="frequency-chart" role="img" aria-label="${chartLabel}">
        ${values.map((value, index) => {
          const week = index + 1;
          const height = value === 0 ? 0 : Math.max(12, Math.round((value / maximum) * 100));
          const weekLabel = L ? `Tydz. ${week}` : `Week ${week}`;
          return `
            <div class="frequency-column" aria-hidden="true">
              <span class="frequency-value">${value}</span>
              <span class="frequency-track" aria-hidden="true">
                <span class="frequency-fill" style="height:${height}%"></span>
              </span>
              <span class="frequency-week">${weekLabel}</span>
            </div>
          `;
        }).join('')}
      </div>
  ...
  _updateUI() {
    ...
    const html = `
      <style>${HA_BACKUP_MANAGER_BENTO_CSS}
...
/* ===== END BENTO LIGHT MODE ===== */
        ...
        .frequency-column {
          display: grid;
          grid-template-rows: 18px 82px auto;
          align-items: end;
